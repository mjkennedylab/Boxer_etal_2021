%---- Requires DipImage toolbox online to run this script
% https://diplib.org/DIPimage.html
%---- Uses a number of methods from Boxer_helperFunctions class. Make sure
%     this is in your Matlab path.
% To Run:
% update topdir and savepath for file locations
% select the file to analyze
% spinning disk notes: RGB is C1,C0,C2

%First thing will be a prompt to enter # of frames subtracted from last frame
%used to do second background subtraction on cell fill channel (enter in
%command)

topdir = '/Users/emmaboxer/Documents/PVsynapsescellfillexcelsheets/Images/New AAV/12.2.20 FWT Images/';
[output,pathname] = uigetfile(fullfile(topdir,'*.*'));
[~,NAME,EXT] = fileparts(output);
basename = NAME(1:end-2);
savepath = '/Users/emmaboxer/Documents/PVsynapsescellfillexcelsheets/2020Analysis/12.2.20 FWT Results';
ext = EXT;
...
ch_cellfillpath = fullfile(pathname,[basename ch_cellfillstr ext]);
ch_tag1path = fullfile(pathname,[basename ch_tag1 ext]);
ch_tag2path = fullfile(pathname,[basename ch_tag2 ext]);
...
prompt= 'select frame for cell fill background' ;
ee=input(prompt);
%% Mask the Cell Fill 
...
[~ ,threshval_g,~]= GeneralAnalysis.imgThreshold_fixedUserInput(cellfill_g(:,:,ceil(numslices)-ee)); 
...
tag1_l = Boxer_helperFunctions.imgLaplaceCutoff(tag1.image); %laplace cuttoff filter
[~ ,threshval_t1,C]= Boxer_helperFunctions.imgThreshold_fixedUserInput(tag1_l(:,:,ceil(numslices/2))); %user select
tag1.mask = tag1_l>threshval_t1;
Boxer_helperFunctions.viewMaskOverlay(tag1.image,tag1.mask);
%dipshow(tag1.image)
%% clean up ch1 (if you want)
%tag1.mask = Boxer_helperFunctions.cleanUpMask_manual_square(dip_image(tag1.image),tag1.mask,100);
%% select ch2 threshold
tag2_l = Boxer_helperFunctions.imgLaplaceCutoff(tag2.image); %laplace cuttoff filter
...
%tag2.mask = GeneralAnalysis.cleanUpMask_manual_square(dip_image(tag2.image),tag2.mask,100);
%% label tag1 puncta
...
%joinchannels('rgb',soma.mask,tag1.colocWtag2.somamask)
%GeneralAnalysis.viewMaskOverlay(cellfill.image,tag2.mask)
%joinchannels('rgb',soma.mask(:,:,ceil(size(soma.mask,3)/2)),tag1.colocWtag2.somamask(:,:,size(tag1.colocWtag2.somamask,3)/2))
%%joinchannels('rgb',soma.mask(:,:,ceil(size(soma.mask,3)/2)),tag1.colocWtag2.somamask(:,:,size(tag1.colocWtag2.somamask,3)/2))
